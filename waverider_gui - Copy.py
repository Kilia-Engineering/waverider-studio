...
try:
    from cone_waverider_tab import ConeWaveriderTab
...
class WaveriderGUI(QMainWindow):
    ...
    def create_visualization_panel(self):
        ...
        if CONE_WAVERIDER_AVAILABLE:
            self.cone_waverider_tab = ConeWaveriderTab(parent=self)
            self.tab_widget.addTab(self.cone_waverider_tab, "🔷 Cone Waverider")
    ...
    def export_cad(self):
        """Export the waverider to CAD format"""
        if self.waverider is None:
            QMessageBox.warning(self, "No Waverider", 
                              "Please generate a waverider first!")
            return
        
        try:
            filename, _ = QFileDialog.getSaveFileName(
                self, "Export Waverider", 
                "/mnt/user-data/outputs/waverider.step",
                "STEP Files (*.step *.stp);;All Files (*)"
            )
            
            if filename:
                self.info_label.setText("Exporting CAD file... Please wait.")
                QApplication.processEvents()
                ...
                to_CAD(
                    ...
                    export=True,
                    filename=filename,
                    scale=1  # Convert to mm
                )
                
                QMessageBox.information(self, "Export Successful",
                    f"Waverider exported to:\n{filename}")
                self.info_label.setText(f"✓ CAD file exported to: {filename}")
    ...
    def export_cad(self):
        ...
        try:
            self.info_label.setText("Exporting CAD file...")
            QApplication.processEvents()
            ...
            to_CAD(
                ...
                export=True,
                filename=filename,
                scale=1.0  # 1.0 = meters (native geometry units)
            )

            QMessageBox.information(
                self, "Export successful",
                f"STEP file exported to:\n{filename}\n\n"
                f"Units: METERS (scale=1.0)\n\n"
    ...
    def generate_stl_mesh(self):
        ...
        try:
            ...
            to_CAD(
                ...
                export=True,
                filename=temp_step,
                scale=1.0  # 1.0 = meters (geometry units)
            )
